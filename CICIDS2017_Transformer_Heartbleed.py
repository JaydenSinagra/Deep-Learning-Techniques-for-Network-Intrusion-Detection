...
file = pd.read_csv('other_alg_new_dataset.csv')
print(file['Attack_Label'].value_counts())

import random
from sklearn.utils import shuffle

# Removing 'DoS GoldenEye' attacks
file = file[file['Attack_Label'] != 'DoS GoldenEye']

benign_total = len(file[file['Attack_Label'] == "BENIGN"])
attack_total = len(file[file['Attack_Label'] != "BENIGN"])
...
# Shuffling data
file_balanced = shuffle(file_balanced, random_state = 42)

file_shuffled['Attack_Label'].value_counts()

file_shuffled.to_csv("heartbleed_balanced.csv", index = False)

file = pd.read_csv('heartbleed_balanced.csv')
